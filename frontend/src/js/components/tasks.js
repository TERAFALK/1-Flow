...
//
// `base` (t.ex. /customers/7) används bara för att skapa och skriva ut. Att
// uppdatera och ta bort går via den globala /api/tasks/{id}, som fungerar oavsett
// om uppgiften hänger på en arbetsorder, en offert eller en kund – annars hade
// varje förälder behövt en egen uppsättning rutter.
...
export function bindTasks(base, tasks, reload) {
  ...
  document.querySelectorAll('[data-task-toggle]').forEach(cb => {
    cb.addEventListener('change', async () => {
      try {
        await api.put(`/tasks/${cb.dataset.taskToggle}`, { completed: cb.checked });
  ...
  document.querySelectorAll('[data-task-del]').forEach(btn => {
    btn.addEventListener('click', async () => {
      ...
      try {
        await api.delete(`/tasks/${btn.dataset.taskDel}`);
